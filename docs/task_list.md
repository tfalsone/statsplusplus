...
- [ ] **SP FV undervaluation** — Pitchers with above-average stuff/movement/control (60/60/55 potential on 20-80) produce a composite of only 60 and ceiling of 53 for a POT 65 player. The composite formula compresses pitcher tools because the weighted average + arsenal bonus doesn't reach the same range as the game's OVR/POT. A pitcher like Schwarzenberg (24yo AAA, pot stf 60/mov 60/ctrl 55, 5-pitch arsenal with 80 cutter) should project as a mid-rotation SP but gets FV 45. Need to investigate: (1) whether the pitcher composite formula systematically underweights something the game values (arsenal depth? pitch quality? pitch mix synergy?); (2) whether a pitcher-specific scaling or floor would close the gap without inflating RP/low-ceiling SP; (3) whether the ceiling age-blend is too aggressive for pitchers specifically (currently 0.60 at age 24 — 40% of ceiling is current composite). **LOE: Medium.**
- [ ] **Per-league dynamic aging/development curves** — Current `_AGE_RUNWAY` tables are hardcoded from cross-sectional analysis. OOTP leagues have game settings that alter development speed, aging onset, and peak age. Curves trained on one league may not apply to another. Should compute `_AGE_RUNWAY_HITTER` and `_AGE_RUNWAY_PITCHER` dynamically per league during calibration, stored in `model_weights.json` alongside tool weights. Implementation: (1) add `calibrate_age_curves()` to `calibrate.py` that runs the cross-sectional OVR/POT gap analysis per league; (2) store results in `model_weights.json`; (3) `fv_model.py` loads league-specific curves at startup, falls back to current hardcoded defaults. **LOE: Medium.**

...
- [ ] **Surface tool_only_score on MLB player pages** — MLB players have both a pure tool composite (`tool_only_score`) and a stat-blended composite (`composite_score`). Currently only the blended score is displayed. Show both so users can see the tools-vs-performance comparison. **LOE: Low.**
- [ ] **Risk labels in prospect list templates** — Risk data is in the queries (added Session 51) but not yet rendered in the league prospect list or team farm tab tables. **LOE: Low.**

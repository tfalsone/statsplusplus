...
- [ ] **SP FV undervaluation** — Pitchers with above-average stuff/movement/control (60/60/55 potential on 20-80) produce a true_ceiling of only 59 for a POT 65 player. The pitcher composite formula compresses tools because the weighted average + arsenal bonus doesn't reach the same range as the game's OVR/POT. Schwarzenberg (24yo AAA SP, 5-pitch arsenal with 80 cutter) gets FV 45 — limited by ceiling compression, not the FV formula. Need to investigate: (1) whether the pitcher composite systematically underweights arsenal depth/quality or pitch mix synergy; (2) whether a pitcher-specific scaling would close the 6-point ceiling-POT gap without inflating RP/low-ceiling SP. The FV system is now well-calibrated — this is a composite/ceiling issue. **LOE: Medium.**
- [ ] **Per-league dynamic aging/development curves** — Current gap closure rates and age decay tables are hardcoded from VMLB cross-sectional analysis. OOTP leagues have game settings that alter development speed, aging onset, and peak age. Should compute `_GAP_CLOSURE_HITTER` and `_GAP_CLOSURE_PITCHER` dynamically per league during calibration, stored in `model_weights.json`. Implementation: (1) add `calibrate_development_curves()` to `calibrate.py`; (2) store in `model_weights.json`; (3) `fv_model.py` loads league-specific curves at startup, falls back to hardcoded defaults. **LOE: Medium.**
- [ ] **FV 45+ inflation residual** — FV 45+/org is 11-19 vs Fangraphs ~8.3. Driven by composite-OVR divergence (+7-8 for mid-range prospects). Our tool-weighted composite is more generous than the game's OVR for prospects with composite 35-50. Not an FV formula issue — needs composite calibration (tool weight tuning or understanding what OVR captures that our composite doesn't). **LOE: Medium.**
- [ ] **Composite-OVR divergence investigation** — Prospect composites run +7-8 above OVR. The gap scales with composite level (+1.8 at comp 20-29, +8.1 at comp 40-44). May indicate the game's OVR includes a "proven-ness" factor our tool composite doesn't capture, or our tool weights are miscalibrated for the mid-range. **LOE: Medium.**

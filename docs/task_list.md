...
- [ ] **Arb salary calibration broken** — `model_weights.json` ARB_PCT values are non-monotonic ({1: 0.71, 2: 0.34, 3: 0.45}) and far from defaults ({1: 0.20, 2: 0.22, 3: 0.33}). Arb 1 at 71% of market value is too high; the non-monotonic pattern is a calibration artifact from small/noisy sample. Causes prospect surplus year 4-6 salary projections to be erratic. Needs: re-examine calibration methodology, validate against actual OOTP arb outcomes, possibly revert to defaults or recalibrate with stricter filters. **LOE: Medium. Priority: High.**
- [ ] **Prospect WAR projection methodology** — Current approach uses `FV_TO_PEAK_WAR` lookup table (FV 50 → 2.0 WAR) with aging curve and ramp. Now that we have composite scores, ceiling profiles, and comp-based validation, consider whether WAR projections should use composite→WAR mapping directly instead of FV→WAR. Would make surplus more granular (two FV 50 prospects with different composites would get different surplus). Needs: compare current FV-based projections against comp-validated WAR ranges, decide if composite-based is more accurate. **LOE: Medium. Priority: Medium.**

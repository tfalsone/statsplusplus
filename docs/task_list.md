...
- [ ] **Scarcity curve refinement** — remaining issue from Session 31: **position-blind scarcity** — a Pot 55 RP gets the same scarcity as a Pot 55 SP, but SP talent is harder to replace and more impactful. Scarcity should either be position-adjusted or the FV→WAR gap between positions should be wide enough that scarcity can't override positional value differences. Observed: FV 45+ RP (Reynolds, Pot 55) valued at $26.1M vs FV 45 SP (Donovan, Pot 48) at $32.3M — ordering is correct but gap may be too narrow. Investigate after positional calibration to avoid double-counting. **LOE: Medium.**
...
- [ ] **Rookie-eligible players in prospect rankings** — players with <100 MLB games (or similar threshold) should appear in top prospect lists even if they're currently at the MLB level. Currently `fv_calc.py` splits strictly on `level == 1` (MLB → `player_surplus`) vs non-MLB (→ `prospect_fv`), so a just-called-up rookie disappears from prospect rankings immediately. Need to: (1) include rookie-eligible MLB players in `prospect_fv` with FV grades; (2) decide whether their surplus should be calculated as prospect surplus (FV-based) or contract surplus (stat-based), or the higher of the two; (3) ensure they appear on farm system pages and top-100 lists. **LOE: Medium.**

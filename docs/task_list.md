...
- [ ] **Historical fielding stats** — Currently only prior year fielding is fetched (matching batting/pitching prior-year fix). Multi-year fielding history would enable: (1) player page fielding history display, (2) more accurate multi-year position assignment for utility players, (3) defensive trend analysis. Low priority since most queries only need the latest year. **LOE: Low.**

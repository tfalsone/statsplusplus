...
- [ ] **Org needs for perpetual arb leagues** — Current `compute_org_needs` is disabled for perpetual arb leagues (no FA departures). Implement an alternative needs calculation based on current roster weakness + farm depth: flag positions where MLB starter composite is below league average AND farm has no FV 50+ replacement. This would give the Need slider meaningful effect in non-standard FA leagues. **LOE: Low-Medium.**
...
- [ ] **League-aware defensive viability in bucket assignment** — `assign_bucket()` uses universal thresholds (e.g., PotSS ≥ 50 = SS) regardless of league environment. In defense-heavy leagues (low K/9, more balls in play), a 55 PotSS with 55 IFR is not viable at SS — you need 65+ range minimum. Options: (1) add configurable defensive floor per position to `league_settings.json` (e.g., `ss_min_ifr: 65`), (2) gate the SS/CF bucket assignment on IFR/OFR + speed meeting a league-specific threshold, (3) use calibration data to derive what IFR/OFR correlates with positive defensive WAR at each position. This would prevent misleading bucket assignments in leagues where defensive standards differ from the default. **LOE: Medium.**
...
- [ ] **SP stamina risk in draft valuation** — SP prospects with stm < 40 (reliever risk) are only lightly penalized in the evaluation engine (-0.75 composite points at stm 35) and not penalized in ceiling/FV at all. The `draft_value()` formula has no stamina check. For young pitchers (17-18), stm is highly developable so light penalty is defensible. For older SP (21+) with low stm, the reliever risk is more concrete. Investigate: (1) whether a stamina penalty in `draft_value` would double-count with the composite penalty, (2) whether age-gating makes sense (no penalty under 19, scaling penalty 19+), (3) impact on PPL specifically where low-stm SP are more viable due to league environment. **LOE: Low-Medium.**

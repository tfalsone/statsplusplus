...
- [ ] **Stale players on org overview page** — Players who have been traded/released/DFA'd in a prior season still appear on the organization page. Other views (depth chart, hitters, pitchers) show correct current rosters. Likely the org overview query uses a broader filter (e.g., `parent_team_id` without checking if the player is still on the team) or doesn't account for mid-season roster changes. Needs investigation into which query produces the stale results. **LOE: Low. Priority: High (data integrity).**
...
- [ ] **Draft board: position filter breaks sorting** — Clicking "Hitters", "Pitchers", or a position filter button kills the table sort functionality. User must reload to restore sorting. Likely conflict between row visibility toggling and `sort.js` state. **LOE: Low. Priority: High (UX broken).**

...
- [ ] **Evaluation model documentation** — Write a comprehensive reference doc (`docs/evaluation_model.md`) with full pipeline diagram, formulas, constants, and design rationale. Cover: tool transform, compensation, composite computation, ceiling, stat blending, FV grading, and surplus. Useful for onboarding and future tuning sessions. **LOE: Low.**
...
- [ ] **Draft prospect ranking** — sort by FV (primary) then surplus (secondary). Web UI already uses this sort. Remaining: review RP bucketing for draft-age arms. **LOE: Low.**

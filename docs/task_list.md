...
- [ ] **Discord integration** — Set up a Discord channel for Stats++ users (development updates, feedback, feature requests). Investigate Discord bot/webhook options: (1) outbound webhooks to post patch notes and changelog updates automatically, (2) inbound integration to read feature requests and bug reports from a dedicated channel, feeding them into our backlog/sessions. Research: Discord bot API, webhook posting, message reading permissions, structured feedback parsing. **LOE: Medium (research + setup).**
